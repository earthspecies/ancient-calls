...
```bash
cd ancient-calls            # unzip the supplementary archive, then cd into it
...
ancientcalls-run --config configs/example.yaml     # full run (needs R)
...
Then run the same three steps as above (`ancientcalls-run` → `make_figures.py` →
...
| path | what |
|------|------|
| `ancientcalls/` | the installable pipeline package |
